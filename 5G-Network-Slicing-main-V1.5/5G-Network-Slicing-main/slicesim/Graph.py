...
class Graph:
    ...
    def calc_max_interruption(self, data, baseline_data, threshold_ratio=0.95):
        ...
        avg_base = np.mean(baseline_data) if baseline_data else 1.0
        if avg_base < 0.1: avg_base = 0.9

        for i in range(length):
            baseline_val = max(baseline_data[i], avg_base * 0.8)
    ...
    def draw_final_report(self, stats_norm, stats_fault, stats_res, duration, last_scenario_objects, slice_config):
        ...
        for idx, sn in enumerate(target_slices):
            ...
            if 'embb' in sn:
                ...
                ax_delta.fill_between(x_axis, delta_series, 0, where=(delta_series >= 0), color='#2ca02c', alpha=0.3,
                                      label='性能增益')
                ax_delta.fill_between(x_axis, delta_series, 0, where=(delta_series < 0), color='#d62728', alpha=0.4,
                                      label='性能损失')
            ...
            ax_table.axis('off')
            self.draw_single_slice_table(ax_table, sn, display_name, (dict_norm, dict_fault, dict_res), win_fault, l, r)
    ...
    def draw_single_slice_table(self, ax, sn, display_name, all_stats, fault_windows, l, r):
        ...
        actual_windows = fault_windows.get(sn, [(0, len(target_n))]) if isinstance(fault_windows, dict) else [(0, len(target_n))]
        evaluator = ResilienceEvaluator()
        res_f = evaluator.evaluate(target_f, actual_windows, target_series=target_n)
        res_r = evaluator.evaluate(target_r, actual_windows, target_series=target_n)
        ...
        if 'embb' in sn:
            ...
            gain_color = ["#ffffff"] * 7
            if diff_g > 1.0: gain_color[5] = "#e6f7ff"; gain_color[6] = "#e6f7ff"
        ...
        table = ax.table(cellText=rows, cellColours=cell_colors, colLabels=columns, loc='center', cellLoc='center',
                         bbox=[0, 0, 1, 1])
        table.auto_set_font_size(False);
        table.set_fontsize(10);
        table.scale(1, 1.8)
        for (row, col), cell in table.get_celld().items():
            if row == 0: cell.set_text_props(weight='bold'); cell.set_facecolor('#d0d0d0')
    ...
    def save_data_to_csv(self, stats_norm, stats_fault, stats_res, slice_config):
        ...
        with open('simulation_summary.csv', 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.writer(f)
            ...
            for sn in target_slices:
                ...
                if 'embb' in sn:
                    th_n, th_f, th_r = mean(d_n['throughput'][l:r]), mean(d_f['throughput'][l:r]), mean(
                        d_r['throughput'][l:r])
                    writer.writerow([sn, 'Mean Throughput (Mbps)', f"{th_n:.2f}", f"{th_f:.2f}", f"{th_r:.2f}",
                                     f"{th_r - th_f:+.2f}"])
                elif 'urllc' in sn:
                    int_n = self.calc_max_interruption(d_n['availability'][l:r], d_n['availability'][l:r])
                    int_f = self.calc_max_interruption(d_f['availability'][l:r], d_n['availability'][l:r])
                    int_r = self.calc_max_interruption(d_r['availability'][l:r], d_n['availability'][l:r])
                    writer.writerow(
                        [sn, 'Max Interruption (s)', f"{int_n}", f"{int_f}", f"{int_r}", f"{int_f - int_r:+.1f}"])
                elif 'mmtc' in sn:
                    mc_n, mc_f, mc_r = mean(d_n['connected_count'][l:r]), mean(d_f['connected_count'][l:r]), mean(
                        d_r['connected_count'][l:r])
                    writer.writerow([sn, 'Mean Connected Count', f"{mc_n:.2f}", f"{mc_f:.2f}", f"{mc_r:.2f}",
                                     f"{mc_r - mc_f:+.2f}"])

        with open('simulation_timeseries.csv', 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.writer(f)
            headers = ['Time_Sec']
            cols_map = []
    ...
    def draw_ablation_bar_chart(self, ablation_results):
        ...
        self._plot_single_ablation(ax_embb, ablation_results['embb'], 'embb',
                                   metric_func=lambda d: np.mean(self.smooth_data(d['throughput'], 20)),
                                   y_label='平均总吞吐量 (Mbps)', title='eMBB: 韧性机制消融分析', base_color='#2ca02c')

        ax_urllc = plt.subplot(gs_bottom[1])
        norm_key = next((k for k in ablation_results['urllc'].keys() if 'Normal' in k), None)
        norm_data = []
        if norm_key: norm_data = ablation_results['urllc'][norm_key]['urllc']['availability']
        ...
        self._plot_single_ablation(ax_urllc, ablation_results['urllc'], 'urllc', metric_func=calc_urllc_metric,
                                   y_label='最大中断时长 (s)', title='URLLC: 中断恢复能力分析', base_color='#1f77b4',
                                   minimize=True)
        ...
        self._plot_single_ablation(ax_mmtc, ablation_results['mmtc'], 'mmtc',
                                   ...
                                   y_label='平均在线连接数', title='mMTC: 拥塞接入能力分析', base_color='#ff7f0e')
    ...
    def _plot_single_ablation(self, ax, data_map, sn, metric_func, y_label, title, base_color, minimize=False):
        ...
        for k in sorted_keys:
            ...
            val = metric_func(slice_data) if slice_data else 0
            clean_label = k.split('.')[-1];
            labels.append(clean_label);
            values.append(val)
            if 'Normal' in k:
                colors.append('#D3D3D3')
            elif 'Faulty' in k:
                colors.append('#EE0000')
            elif 'All' in k:
        ...
        for bar in bars:
            ...
            ax.text(bar.get_x() + bar.get_width() / 2., height + offset, f'{height:.1f}', ha='center', va=va,
