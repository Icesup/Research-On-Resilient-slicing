...
def fault_urllc_outage(env, base_stations, clients, stats, rng, sim_time):
    ...
    while True:
        ...
        print(f"[Fault-URLLC] T={env.now:.1f}: Regional Failure Simulation.")
        for bs in victims:
            bs.active = False
        ...
        print(f"[Recovery-URLLC] T={env.now:.1f}: Nodes Recovered (Clients Reconnected: {reconnect_count}).")
...
def run_scenario(config_data, enable_resilience=True, enable_faults=False, base_seed=42, fault_seed=None):
    ...
    for i in range(SETTINGS['num_clients']):
        ...
        # 注意：这里没有传 map_limits，因为您之前的 Client 代码似乎不需要。
        # 如果您的 Client.py 被修改过需要该参数，请自行添加。
