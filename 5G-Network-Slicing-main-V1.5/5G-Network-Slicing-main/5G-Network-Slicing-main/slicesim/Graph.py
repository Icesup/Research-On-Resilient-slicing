...
class Graph:
    def __init__(self, output_filename='comparison_report_final.png'):
        ...
        # 主布局：左边地图，右边图表区
        # width_ratios 控制左右比例，height_ratios 控制每一行的高度
    ...
    def draw_final_report(self, stats_norm, stats_fault, stats_res, duration, last_scenario_objects, slice_config):
        ...
        for idx, sn in enumerate(target_slices):
            ...
            # === eMBB 特殊处理：双子图 (左：吞吐量，右：性能差值) ===
            # 这将直接回答您"为什么会有损失/增益"的问题
            if 'embb' in sn:
                # 创建嵌套 GridSpec，1行2列，填满原来的图表区域
                # width_ratios=[1, 1] 让左右图等宽
                ...
                # 2. 右图：性能差值分析 (Resilient - Normal)
                # 这里将清晰展示：哪里是赚的(绿)，哪里是亏的(红)
                ...
                # 填充 Gain 区域 (绿色) -> 对应表格 Gain Area
                ax_delta.fill_between(x_axis, delta_series, 0, where=(delta_series >= 0),
                                      color='#2ca02c', alpha=0.3, label='性能增益 (Res > Norm)')

                # 填充 Loss 区域 (红色) -> 对应表格 Loss Area
                ax_delta.fill_between(x_axis, delta_series, 0, where=(delta_series < 0),
                                      color='#d62728', alpha=0.4, label='性能损失 (Res < Norm)')
                ...
                # 确保两图与下方表格对齐 (通过 GridSpec 已经隐式保证，这里只是微调坐标轴显示)
                ax_chart.set_xlim(l, r)
                ax_delta.set_xlim(l, r)

            else:
                # === 其他切片 (URLLC/mMTC)：保持单大图 ===
                ax_chart = plt.subplot(self.gs[row_chart, 1])

                ax_chart.plot(x_axis, y_n, color='gray', linestyle=':', linewidth=1.5, alpha=0.7,
                              label='正常组 (Normal)')
    ...
    def draw_single_slice_table(self, ax, sn, display_name, all_stats, fault_windows, l, r):
        ...
        if 'embb' in sn:
            ...
            # 3. 性能损失面积 (Loss Area)
            # 逻辑修正：Faulty(大损失) - Resilient(小损失) = 挽回的损失(正数)
            ...
            # URLLC 中断时长：Faulty - Resilient = 缩短的时间(正数)
            ...
            bk_f = mean(blk_f_seq)
            bk_r = mean(blk_r_seq)
            # 阻塞率：Faulty - Resilient = 降低的阻塞率(正数)
            diff = bk_f - bk_r
        ...
        with open(summary_file, 'w', newline='', encoding='utf-8-sig') as f:
            ...
            for sn in target_slices:
                ...
                if 'embb' in sn:
                    ...
                    if 'blocking' in d_n:
                        blk_n = d_n['blocking'][l:r]
                        blk_f = d_f['blocking'][l:r]
                        blk_r = d_r['blocking'][l:r]
                        mb_n, mb_f, mb_r = mean(blk_n), mean(blk_f), mean(blk_r)
                        writer.writerow(
                            [sn, 'Mean Blocking', f"{mb_n:.3f}", f"{mb_f:.3f}", f"{mb_r:.3f}", f"{mb_f - mb_r:+.3f}"])
