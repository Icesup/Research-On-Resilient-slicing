...
def fault_embb_degradation(env, base_stations, stats, rng, sim_time):
    ...
    while True:
        ...
        print(f"[Fault-eMBB] T={env.now:.1f}: Rain Fade (Capacity Degradation).")
        ...
        print(f"[Recovery-eMBB] T={env.now:.1f}: Capacity restored.")


def fault_urllc_outage(env, base_stations, stats, rng, sim_time):
    yield env.timeout(40)
    while True:
        interval = rng.randint(40, 70)
        yield env.timeout(interval)
        if env.now > sim_time - 20: break

        victim_count = rng.randint(5, 8)
        victims = rng.sample(base_stations, k=victim_count)

        print(f"[Fault-URLLC] T={env.now:.1f}: {len(victims)} Satellites Failure.")
        for bs in victims:
            bs.active = False
        ...
        print(f"[Recovery-URLLC] T={env.now:.1f}: Satellites rebooted.")
