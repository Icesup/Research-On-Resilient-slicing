...
class Graph:
    ...
    def draw_single_slice_table(self, ax, sn, display_name, all_stats, fault_windows, l, r):
        ...
        if 'embb' in sn:
            ...
            diff = la_f - la_r
            concl = self._get_conclusion(diff, 'higher_good', labels=("损失减小", "损失增加"), threshold=0.1)
    ...
    def save_data_to_csv(self, stats_norm, stats_fault, stats_res, slice_config):
        ...
        with open(summary_file, 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.writer(f)
            writer.writerow(['Slice', 'Metric', 'Normal_Val', 'Faulty_Val', 'Resilient_Val', 'Diff(Res-Fault)'])
            ...
            for sn in target_slices:
                ...
                if 'embb' in sn:
                    ...
                    int_r = self.calc_max_interruption(conn_r, conn_n)
                    writer.writerow(
                        [sn, 'Max Interruption (s)', f"{int_n}", f"{int_f}", f"{int_r}", f"{int_f - int_r:+}"])

                elif 'mmtc' in sn:
                    # [关键] mMTC: 导出平均连接数 (Connected Count)
                    key = 'connected_count'
                    ...
                    if key in d_n and d_n[key]:
                        ...
                        print(f"[Warning] No connected_count data found for {sn}")
