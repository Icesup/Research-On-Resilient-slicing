...
class Graph:
    ...
    def smooth_data(self, data, window_size=30):
        if not data: return []
        # [关键修正] 允许关闭平滑 (window_size=1) 以展示尖峰
        if len(data) < window_size or window_size < 2: return data
        window = np.ones(int(window_size)) / float(window_size)
        return np.convolve(data, window, 'same')

    def calc_max_interruption(self, data, baseline_data, threshold_ratio=0.95):
        """
        [关键修正] 相对基准判定：只有显著低于正常组才算中断
        """
        max_duration = 0
        current_duration = 0
        length = min(len(data), len(baseline_data))
        for i in range(length):
            baseline_val = max(baseline_data[i], 0.01)  # 防止除零
    ...
    def _get_conclusion(self, diff, mode='higher_good', labels=None, threshold=0.001):
        ...
        # 统一逻辑：diff > 0 代表数值增加。需结合指标方向判断好坏。
        if mode == 'higher_good':
    ...
    def draw_final_report(self, stats_norm, stats_fault, stats_res, duration, last_scenario_objects, slice_config):
        ...
        for idx, sn in enumerate(target_slices):
            ...
            if 'embb' in sn:
                ...
                y_label = '总吞吐量 (Mbps)'
                line_color = '#2ca02c'
                w_size = 30  # eMBB 需要平滑看趋势
            elif 'urllc' in sn:
                data_key = 'availability'
                y_label = '瞬时可用性 (0-1)'
                line_color = '#1f77b4'
                w_size = 1  # URLLC 禁止平滑，看跌落
            elif 'mmtc' in sn:
                data_key = 'blocking'
                y_label = '接入阻塞率 (0-1)'
                line_color = '#ff7f0e'
                w_size = 1  # mMTC 禁止平滑，看尖峰
            else:
                data_key = 'availability'
                y_label = '可用性'
                line_color = 'black'
    ...
    def draw_single_slice_table(self, ax, sn, display_name, all_stats, fault_windows, l, r):
        ...
        # [关键修正] eMBB 必须用吞吐量来计算损失面积，其他用可用性
        if 'embb' in sn:
        ...
        # 传入正确的目标数据
        res_f = evaluator.evaluate(target_f, [], target_series=target_n)
        res_r = evaluator.evaluate(target_r, [], target_series=target_n)
