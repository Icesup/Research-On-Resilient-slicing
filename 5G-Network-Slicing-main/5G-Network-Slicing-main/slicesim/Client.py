...
class Client:
    def __init__(self, pk: int, env, x: float, y: float, mobility_pattern,
        ...
        # [消融实验] eMBB/URLLC: 业务优先级设置
        # 只有当 enable_qos 为 True 时，才分配高权重，否则全为 1.0
        enable_qos = self.res_config.get('enable_qos', False)

        if "embb" in self.slice_name.lower():
            if random.random() < 0.4:  # 40% HD Video
                ...
                if enable_qos:
                    self.priority_weight = 0.8

        # URLLC 默认高优
        if "urllc" in self.slice_name.lower():
            if enable_qos:
                self.priority_weight = 3.0
            else:
                self.priority_weight = 1.0
    ...
    def run(self):
        ...
        while True:
            # === [消融实验] eMBB: 主动负载均衡 (Active Load Balancing) ===
            ...
            if int(self.env.now) % 2 == 0 and self.connected and self.base_station:
                ...
                # 只有切片支持备份 且 配置允许主动均衡 时才执行
                if current_slice and current_slice.require_backup and enable_lb:
            ...
            if self.base_station is not None:
                if not self.base_station.active:
                    ...
                    # === [消融实验] eMBB/URLLC/mMTC: 被动快速切换 (Passive Fast Handover) ===
